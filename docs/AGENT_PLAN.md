...
**Files to create:**
- `knowledge-base/parse_wallach.py` — Parse Wallach HTML → chunks.json (READY)
- `knowledge-base/build_kb.py` — Embed chunks + load into LanceDB
...
STEP 0: PARSE HTML (parse_wallach.py — ALREADY BUILT)
   - Input: Wallach HTML files saved via View Source from apn.lwwhealthlibrary.com
   - Extracts: <h2 class='content-section-header'> as chunk boundaries, <div class="para"> as text
   - Auto-detects biomarkers and organ system classification
   - Filters out non-clinical sections
   - Output: knowledge-base/chunks.json
   - Usage: python parse_wallach.py raw/chapter2.html raw/chapter3.html ...

STEP 1: EMBED & LOAD (build_kb.py — run once)
   - Reads chunks.json
   - Embeds with all-MiniLM-L6-v2
   - Loads into LanceDB at knowledge-base/lancedb/

1. SOURCE MATERIAL:
   - Wallach's Interpretation of Diagnostic Tests (primary — organized by clinical pattern)
   - HTML source from Silverchair platform (lwwhealthlibrary.com)
   - Note: Chapter 1 (FALTs) = pre-analytical factors → useful for verification rules
   - Chapters 2+ = clinical patterns → these are the RAG knowledge base

2. CHUNKING STRATEGY (handled by parse_wallach.py):
   Each <h2 class='content-section-header'> = one chunk boundary
   - Chunks follow Wallach's natural clinical pattern organization
   - Metadata auto-detected: biomarkers mentioned, organ systems, word count
   - Target: 200-500 words per chunk, minimum 30 words (drops header-only sections)

3. METADATA (CRITICAL — used for pre-filtering at runtime):
   Auto-generated by parse_wallach.py:
   ```python
   {
       "chunk_id": "wallach_ch02_s001",
       "source": "Wallach's Interpretation of Diagnostic Tests, 11th Edition",
       "chapter": "chapter2",
       "section_title": "Iron Deficiency Anemia — Clinical Features",
       "organ_system": ["hematologic"],  # multi-label: can be >1
       "biomarkers_mentioned": ["ferritin", "MCV", "hemoglobin", "transferrin"],
       "word_count": 387
   }
   ```
...
5. LANCEDB SETUP:
   ...
   table.create_fts_index("biomarkers_mentioned")
   ```
...
- [x] `knowledge-base/parse_wallach.py` — HTML parser built and tested on Ch. 1 structure
- [ ] Medical student: copy HTML source of clinical chapters (Ch. 2+) from Wallach website via View Source
- [ ] Run `python parse_wallach.py raw/chapter*.html` to extract chunks → chunks.json
- [ ] Med student validates chunks for clinical accuracy and correct section boundaries
